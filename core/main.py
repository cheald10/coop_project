...
from app.workflows import process_coop_request
from app.workflows import generate_coop_document
...
@app.post("/coop/request") 
async def handle_request(user_id: int, text: str): 
    result = await dbos.run_workflow(process_coop_request, user_id, text) 
    return {"status": result}

@app.post("/coop/generate-doc")
async def generate_doc(coordinator: str, division: str, details: str): 
    result = await dbos.run_workflow( 
        generate_coop_document, 
        coordinator, 
        division, 
        details 
    ) 
